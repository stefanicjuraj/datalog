    ...
    return (
        <tr className="bg-[#FFF] hover:bg-[#fff] border-b">
            ...
            <td className="sm:pl-0 pl-6">
                {certificate["Topic"] && certificate["Topic"].map((topic, index) => (
...
function Certificates() {
    const [jsonData, setJsonData] = useState<Certificate[]>([]);
    ...
    // filter data by certificates name
    const filterData = (searchTerm: string) => {
        const filteredData = jsonData.filter((certificate) =>
    ...
    const sortData = () => {
        const sortedData = [...filteredData].sort((a, b) => {
            const aValue = a["Certificate"].toLowerCase();
            const bValue = b["Certificate"].toLowerCase();
                    ...
                    <div className="mt-24 sm:px-4 px-0 mx-auto max-w-screen-xl">
                        ...
                        <div className="mb-4 sm:p-16 p-8 bg-[#222] rounded-3xl text-left shadow-sm shadow-black">
                            ...
                            <p className="w-full sm:w-3/4 text-lg mt-2 text-white">
                                A curated datalog of IT (ICT) <span className="bg-[#333] px-1 py-0.3 rounded-lg">certificates</span> {" "} obtainable in Croatia.
                            </p>
                        </div>
                    </div>
                        ...
                        <div className="flex items-center">
                            ...
                            <Link to="/contribute" className="sm:text-lg text-md sm:mt-0 mt-4 animation glow delay-1">
                                <h1 className="mr-3 sm:text-lg text-md text-[#AAA] hover:text-black whitespace-nowrap">
                                    Can't find your favorite certificates?{' '}
                            ...
                            <tbody id="companyTableBody" className="animation glow delay-3">
                                ...
                                {isLoading ? (
                                    ...
                                    filteredData.length === 0 ? (
                                        ...
                                        filteredData.map((certificate) => (
                                            <CertificateTable key={certificate["Certificate"]} certificate={certificate} />
                                        ))
                                    )
                                )}
                            </tbody>
